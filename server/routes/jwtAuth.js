...
router.post("/inscription",validInfo, async (req, res) => {
    try {
        ...
        if (password !== password2) {return res.status(401).json("Les deux mots de passe sont différents.")};
        ...
        if (user_pseudo.rows.length !== 0) {return res.status(401).json("Pseudo déjà utilisé.")};
        if (user_mail.rows.length !== 0) {return res.status(401).json("Adresse mail déjà utilisée.")};
